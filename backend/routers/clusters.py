...
def list_clusters(ds: Dataset = Depends(get_dataset)) -> list[ClusterOut]:
    ...
    for r in ds.clusters.itertuples():
        ...
        top_gids = [ds.external_id(g) for g in top_gids_raw.split(";") if g]
...
def list_top_nodes(ds: Dataset = Depends(get_dataset)) -> list[TopNodeOut]:
    return [
        TopNodeOut(rank=int(r.rank), gid=ds.external_id(r.gid), role=r.role,
                   priority_score=float(r.priority_score), risk_score=float(r.risk_score),
                   risk_level=r.risk_level, why=r.why)
