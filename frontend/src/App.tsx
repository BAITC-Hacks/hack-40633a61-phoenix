...
import {
  Activity, ArrowDownRight, ArrowRight, ArrowUpRight, Bell, Bot, ChevronDown,
  ChevronRight, CircleHelp, Database, Download, Filter,
  FileUp, Fingerprint, Focus, Home, Layers3, Menu, MoreHorizontal, Network,
  Search, ShieldAlert, SlidersHorizontal, Sparkles, X,
} from 'lucide-react'
import { roleColors, type GraphNode, type Risk, type Role } from './data'
import { ApiError, askAi, fetchAiStatus, uploadAnalysis, type AnalysisResponse } from './api'
import { useGraphData } from './store'

type Page = 'Overview' | 'Network' | 'Priority' | 'Clusters' | 'Investigation' | 'Data' | 'Assistant'
const pages: { name: Page; icon: typeof Home }[] = [
  ...
  { name: 'Investigation', icon: Search }, { name: 'Data', icon: FileUp },
...
function Graph({ selected, onSelect, roleFilter, clusterFilter, compact = false }: {
  ...
  const { nodes, transfers } = useGraphData()
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({
      ...
      elements: [
        ...nodes.map((node) => ({ data: { id: node.gid, label: node.name, role: node.role, color: roleColors[node.role], score: node.score } })),
        ...transfers.map((transfer) => ({ data: { id: transfer.id, source: transfer.source, target: transfer.target } })),
      ],
      layout: {
        name: 'concentric', fit: true, padding: compact ? 38 : 65,
        concentric: (node) => Number(node.data('score')), levelWidth: () => 8,
        minNodeSpacing: compact ? 8 : 16, animate: false,
      },
      style: [
  ...
  }, [selected, roleFilter, clusterFilter, nodes])
...
function Overview({ onNavigate, onSelect }: { onNavigate: (page: Page) => void; onSelect: (gid: string) => void }) {
  const { nodes, transfers, clusters } = useGraphData()
...
function PriorityPage({ onSelect, onNavigate }: { onSelect: (gid: string) => void; onNavigate: (page: Page) => void }) {
  const { nodes, clusters } = useGraphData()
  const [query, setQuery] = useState('')
...
function DataPage({ onNavigate }: { onNavigate: (page: Page) => void }) {
  const { analysisId, selectAnalysis } = useGraphData()
  ...
  const submit = async () => {
    if (!files.length) { setError('Choose one to three parquet files.'); return }
    setState('uploading')
    setError('')
    window.setTimeout(() => setState((current) => current === 'uploading' ? 'analyzing' : current), 250)
    try {
      const response = await uploadAnalysis(files)
      setResult(response)
      setState('done')
      selectAnalysis(response.analysis_id)
    } catch (err) {
      setState('idle')
  ...
  return <div className="page-content">
    <div className="page-header"><div><Kicker>DATA / ISOLATED RUNS</Kicker><h1>Analyze parquet data</h1><p>Upload validated transaction data without replacing the demo dataset.</p></div><span className="header-chip">{analysisId ? `Run ${analysisId.slice(0, 8)}` : 'Demo dataset active'}</span></div>
    <div className="panel upload-panel">
      <div className="panel-title"><div><h2>Upload parquet</h2><p>Maximum 3 files · 50 MB each · 500,000 rows each</p></div><FileUp size={22} /></div>
      <label className="drop-zone" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); choose(e.dataTransfer.files) }}>
        <FileUp size={31} /><strong>Drop files here or choose files</strong>
        <span>One universal transaction parquet, or nodes + edges + transactions</span>
        <input type="file" accept=".parquet" multiple onChange={(e) => choose(e.target.files)} />
      </label>
      {files.length > 0 && <div className="file-list">{files.map((file) => <span key={`${file.name}-${file.size}`}><Database size={15} />{file.name}<small>{(file.size / 1024 / 1024).toFixed(2)} MB</small></span>)}</div>}
      {error && <div className="upload-error"><ShieldAlert size={17} />{error}</div>}
      <div className="upload-actions"><button className="button button-primary" disabled={!files.length || state === 'uploading' || state === 'analyzing'} onClick={submit}>{state === 'uploading' ? 'Uploading…' : state === 'analyzing' ? 'Running pipeline…' : 'Validate and analyze'} <ArrowRight size={16} /></button>{analysisId && <button className="button button-secondary" onClick={() => selectAnalysis(null)}>Return to demo</button>}</div>
    </div>
    {result && <div className="panel result-panel"><div className="panel-title"><div><h2>Analysis completed</h2><p>{result.results.nodes} nodes · {result.results.edges} edges · {result.results.transactions} transactions</p></div><span className="complete">HEURISTIC RESULTS</span></div>
      <div className="schema-results">{result.files.map((file) => <div key={file.name} className="schema-card"><strong>{file.name}</strong><span>Inferred as {file.kind} · {file.rows.toLocaleString()} rows</span><code>{Object.entries(file.column_mapping).map(([key, value]) => `${value} → ${key}`).join(' · ')}</code><div className="preview-scroll"><table><thead><tr>{file.columns.slice(0, 6).map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{file.preview.slice(0, 3).map((row, index) => <tr key={index}>{file.columns.slice(0, 6).map((column) => <td key={column}>{String(row[column] ?? '—')}</td>)}</tr>)}</tbody></table></div></div>)}</div>
      <div className="limitations"><strong>Confidence: {result.confidence}</strong>{result.limitations.map((item) => <span key={item}>{item}</span>)}</div>
      <button className="button button-primary" onClick={() => onNavigate('Network')}>Open analyzed network <Network size={16} /></button>
    </div>}
  </div>
}

function AssistantPage() {
  const { analysisId } = useGraphData()
  ...
  useEffect(() => {
    fetchAiStatus().then((status) => setConfigured(status.configured)).catch((err) => setError(err instanceof Error ? err.message : 'Status unavailable.'))
  }, [analysisId])
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setCitations(response.citations.map((citation) => citation.gid))
    } catch (err) {
      setError(err instanceof ApiError && err.status === 503 ? 'AI assistant is not configured. Add OPENAI_API_KEY to the server .env.' : err instanceof Error ? err.message : 'Assistant request failed.')
    } finally { setLoading(false) }
  }
  return <div className="page-content"><div className="page-header"><div><Kicker>GROUNDED AML ASSISTANT</Kicker><h1>Transaction assistant</h1><p>Questions are limited to the active analysis, its transactions and risk hypotheses.</p></div><span className="header-chip">{configured === null ? 'Checking…' : configured ? 'AI ready' : 'No API key'}</span></div>
    <div className="panel assistant-panel">
      {!configured && configured !== null && <div className="assistant-notice"><ShieldAlert size={22} /><div><strong>Assistant unavailable</strong><p>Set a real OpenAI API key on the backend. A project URL is not an API key; no secret is ever sent to this browser.</p></div></div>}
      <form onSubmit={submit}><textarea value={question} onChange={(e) => setQuestion(e.target.value)} maxLength={2000} placeholder="Ask about a GID, consolidation, fan-out, pass-through, cycles or temporal bursts…" disabled={!configured || loading} /><div><small>{question.length}/2000</small><button className="button button-primary" disabled={!configured || !question.trim() || loading}>{loading ? 'Analyzing…' : 'Ask assistant'} <Bot size={16} /></button></div></form>
      {error && <div className="upload-error"><ShieldAlert size={17} />{error}</div>}
      {answer && <div className="assistant-answer"><span className="eyebrow">GROUNDED RESPONSE</span><p>{answer}</p>{citations.length > 0 && <div>Sources: {citations.map((gid) => <code key={gid}>gid {gid}</code>)}</div>}</div>}
      <div className="limitations"><strong>Guardrails</strong><span>Only selected pipeline context is sent; context length is capped.</span><span>Uploaded values are treated as untrusted data, never as instructions.</span><span>Findings are AML hypotheses, not accusations.</span></div>
    </div>
  </div>
}

export default function App() {
  const { loading: isLoading, error, reload, analysisId } = useGraphData()
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><div className="top-left"><button className="icon-button mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu size={22} /></button><div className="workspace-switch"><button onClick={() => setShowWorkspace(!showWorkspace)}>AML Investigation Workspace {analysisId ? `· ${analysisId.slice(0, 8)}` : '· Demo'} <ChevronDown size={16} /></button>{showWorkspace && <div className="popover workspace-popover"><span className="eyebrow">CURRENT WORKSPACE</span><strong>{analysisId ? `Analysis ${analysisId.slice(0, 8)}` : 'Demo analysis'}</strong><small>Money Graph intelligence platform</small></div>}</div></div><div className="top-actions"><button className="icon-button notifications" onClick={() => setShowNotifications(!showNotifications)} aria-label="Notifications"><Bell size={20} /></button>{showNotifications && <div className="popover notification-popover"><span className="eyebrow">NOTIFICATIONS</span><strong>No notifications loaded</strong><small>Updates will appear here when available.</small></div>}<span className="top-divider" /><button className="profile-button" onClick={() => setShowProfile(!showProfile)}><span className="avatar">M</span><span>Money Graph</span><ChevronDown size={16} /></button>{showProfile && <div className="popover profile-popover"><span className="eyebrow">WORKSPACE</span><strong>Money Graph</strong><small>Investigation workspace</small></div>}</div></header>
      <main><PageErrorBoundary>{isLoading && page !== 'Data' ? <div className="loading-screen"><BrandMark small /><span>Preparing workspace...</span></div> : error && page !== 'Data' ? <div className="page-error"><ShieldAlert size={30} /><h2>Unable to load analysis</h2><p>{error}</p><button className="button button-primary" onClick={reload}>Retry <ArrowRight size={16} /></button></div> : page === 'Overview' ? <Overview onNavigate={navigate} onSelect={inspect} /> : page === 'Network' ? <NetworkPage selected={selected} setSelected={setSelected} roleFilter={roleFilter} setRoleFilter={setRoleFilter} clusterFilter={clusterFilter} setClusterFilter={setClusterFilter} onNavigate={navigate} /> : page === 'Priority' ? <PriorityPage onSelect={inspect} onNavigate={navigate} /> : page === 'Clusters' ? <ClustersPage setClusterFilter={setClusterFilter} setSelected={setSelected} onNavigate={navigate} /> : page === 'Investigation' ? <InvestigationPage onSelect={inspect} onNavigate={navigate} /> : page === 'Data' ? <DataPage onNavigate={navigate} /> : <AssistantPage />}</PageErrorBoundary></main>
    </div>
  </div>
}
